...
class OAuth2StoragePDO implements IOAuth2GrantCode, IOAuth2RefreshTokens {
	 ...
	 */
	public function addClient($application_name, $client_id, $client_secret, $redirect_uri, $apikey = null) {
		try {
			$client_secret = $this->hash($client_secret, $client_id);
			
			$sql = 'INSERT INTO ' . self::TABLE_CLIENTS . ' (application_name, client_id, client_secret, redirect_uri,apikey) VALUES (:application_name, :client_id, :client_secret, :redirect_uri, :apikey)';
			$stmt = $this->db->prepare($sql);
			$stmt->bindParam(':application_name', $application_name, PDO::PARAM_STR);
			...
			$stmt->execute();
            self::addUser($client_id,$apikey,$redirect_uri);
    ...
    public function addUser($clientId,$apiKey,$redirectUri){
        $redirectUri='user@'.$redirectUri;
        $isCorporate = 1;
        try{
        $sql='insert into '.self::TABLE_USER.' (username, password, email, is_corporate) values (:client_id, :api_key, :redirect_uri, :is_corporate)';
        $stmt = $this->db->prepare($sql);
        ...
        $stmt->bindParam(':is_corporate', $isCorporate, PDO::PARAM_INT);
        $stmt->execute();
        } catch (PDOException $e) {
            $this->handleException($e);
        }
    ...
    public function mapUserClientId($user_id,$client_id){
        try{
            $sql='insert into '.self::TABLE_USER_APP.' (user_id, client_id) values (:user_id, :client_id)';
            $stmt = $this->db->prepare($sql);
    ...
    public function getUserClient($user_id){
        try{
            $sql='select * from '.self::TABLE_USER_APP.' where user_id = :user_id';
            $stmt = $this->db->prepare($sql);
            $stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
            $stmt->execute();
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            return isset($result["client_id"])?$result["client_id"]:null;
        } catch (PDOException $e) {
            $this->handleException($e);
        }
    }
    ...
    public function refreshApiKey($client_id, $apikey){
        try {
            $sql = 'UPDATE ' . self::TABLE_CLIENTS . ' SET apikey = :apikey WHERE client_id = :client_id';
            $stmt = $this->db->prepare($sql);
            $stmt->bindParam(':apikey', $apikey, PDO::PARAM_STR);
            $stmt->bindParam(':client_id', $client_id, PDO::PARAM_STR);
            $stmt->execute();
    ...
    public function refreshClientSecret($client_id, $client_secret){
        $client_secret = $this->hash($client_secret, $client_id);
        try {
            $sql = 'UPDATE ' . self::TABLE_CLIENTS . ' SET client_secret = :client_secret WHERE client_id = :client_id';
            $stmt = $this->db->prepare($sql);
            $stmt->bindParam(':client_secret', $client_secret, PDO::PARAM_STR);
            $stmt->bindParam(':client_id', $client_id, PDO::PARAM_STR);
            $stmt->execute();
    ...
    /**
     * ARB Check ApiKey Function
     * @param $apikey
     * @return bool|null
     */
    public function checkApiKey($apikey) {
        try {
            $sql = 'SELECT redirect_uri FROM ' . self::TABLE_CLIENTS . ' WHERE apikey = :apikey';
            $stmt = $this->db->prepare($sql);
            $stmt->bindParam(':apikey', $apikey, PDO::PARAM_STR);
            $stmt->execute();
